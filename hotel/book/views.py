...
def index(request):
    pop = Hotel.objects.filter(rating='5')
...
def popular_hotels(request):
    popular = Hotel.objects.filter(rating='5')
...
def successful(request):
    ...
    b.save()
    return render(request, "book/successful.html" ,{'tracking_id': tracking_id})
...
def add_hotels(request):
    if request.method == "POST":
        ...
        hotel.highlight = request.POST["highlight"]
        hotel.room_types = request.POST["room_types"]
        hotel.rating = request.POST["rating"]
        hotel.price = request.POST["price"]
        hotel.imgurls = request.POST["imgurls"]
        hotel.type = request.POST["type"]
        hotel.save()

    return render(request, "book/add_hotels.html")
